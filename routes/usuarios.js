...
router.post("/",upload.single("avatar"),function(req,res,next){
	...
	.then(function(d){
		...
		res.status(500)
		.json({message:"Error, nombre de usuario ya existente"});
	})
})
.get("/",function(req,res,next){
	Usuario.findAll({where:res.query})
	.then(function(d){
		if(d){
		res.status(200)
		.json({usuarios:d});}
		else{
			res.status(200).json({message:"No hay usuarios que mostrar"});
		}
	})
})
...
.put("/:id",function(req,res,next){	
	...
	var actualizacion={password:req.body.password};
	Usuario.update(actualizacion,{where:{id_usuario:req.params.id},returning:true})
	.then(function(d){	
		if(d[0]>0){	
		res.status(200)
		.json({usuario:d[1][0]})}
			...
			res.status(200)
			.json({message:"No se actualizo ningun usuario"});
		}
	})
})
.put("/:id/avatar",upload.single("avatar"),function(req,res,next){
	...
	.then(function(d){
		if(d){
			...
			Usuario.update(d,{where:{id_usuario:req.params.id},returning:true})
			.then(function(d){
				if(d[0]>0){
...
.delete("/:id",function(req,res,next){
	...
	.then(function(d){
		if(d){
			...
			.then(function(n){
				res.status(200)
				.json({message:"Se ha eliminado "+n+" usuario"});
			});			
		}else{
